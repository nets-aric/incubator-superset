...
class ServerSideTable(object):
    ...
    def _custom_filter(self, data):
            ...
            for i in range(len(self.columns)):
                ...
                if self.columns[i]['searchable']:
                  if self.request_values.get(search_col, ''):
                    value = row[self.columns[i]['column_name']]
                    regex = '(?i)' + self.request_values[search_col]
                    if re.compile(regex).search(str(value)) and self.request_values[search_col]:
                      matched =1
                    else :
                      return False
        ...
        def glob_multi_check_row(row):
            ''' Checks whether a row should be displayed or not. '''
            for i in range(len(self.columns)):
                if self.columns[i]['searchable']:
                    col_name = 'sSearch_0'
                    value = row[self.columns[i]['column_name']]
                    regex = '(?i)' + self.request_values[col_name]
                    if re.compile(regex).search(str(value)):
                        return True
                else:
                    value = row[self.columns[i]['column_name']]
        ...
        for i in range(len(self.columns)):
            search_col = 'sSearch_' + str(i)
            if self.columns[i]['searchable'] and filter < 2:
                if self.request_values.get(search_col, ''):
                    if self.request_values.get('sSearch', ''):
                        filter = 3
                    else:
                        filter = 1
                elif self.request_values.get('sSearch', ''):
                    filter = 1
        ...
        if filter == 1:
            ...
            print("multi_check_row")
            return [row for row in data if multi_check_row(row)]
        elif filter == 3:
            print("glob_multi_check_row")
            return [row for row in data if glob_multi_check_row(row)]
        else:
    ...
    def _custom_sort(self, data):
        ...
        if self.request_values['iSortCol_0'] != '' \
            ...
            for i in range(0, int(self.request_values['iSortingCols'])):
                column_number = int(self.request_values['iSortCol_'
                                    + str(i)])
                column_name = self.columns[column_number]['column_name']
                sort_direction = self.request_values['sSortDir_'
                        + str(i)]
                data = sorted(data, key=lambda x: x[column_name],
